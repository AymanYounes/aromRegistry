! function(t) {
    ...
    return function() {
        ...
        t.validator.addMethod("maxWords", function(t, a, d) {
            ...
            var i = e(t);
            return this.optional(a) || i.match(/\b\w+\b/g).length >= d[0] && i.match(/\b\w+\b/g).length <= d[1]
        }, t.validator.format("Please enter between {0} and {1} words."))
    }(), t.validator.addMethod("accept", function(e, a, d) {
        var i, n, r, o = "string" == typeof d ? d.replace(/\s/g, "") : "image/*",
            s = this.optional(a);
        if (s) return s;
        if ("file" === t(a).attr("type") && (o = o.replace(/[\-\[\]\/\{\}\(\)\+\?\.\\\^\$\|]/g, "\\$&").replace(/,/g, "|").replace(/\/\*/g, "/.*"), a.files && a.files.length))
            for (r = new RegExp(".?(" + o + ")$", "i"), i = 0; i < a.files.length; i++)
                if (n = a.files[i], !n.type.match(r)) return !1;
        return !0
        ...
        var a, d, i, n, r, o = !1,
            s = /^\d{1,2}\/\d{1,2}\/\d{4}$/;
        return s.test(t) ? (a = t.split("/"), d = parseInt(a[0], 10), i = parseInt(a[1], 10), n = parseInt(a[2], 10), r = new Date(Date.UTC(n, i - 1, d, 12, 0, 0, 0)), o = r.getUTCFullYear() === n && r.getUTCMonth() === i - 1 && r.getUTCDate() === d) : o = !1, this.optional(e) || o
        ...
        if (this.optional(e)) return !0;
        var a, d, i, n, r, o, s, l, u, c = t.replace(/ /g, "").toUpperCase(),
            h = "",
            f = !0,
            p = "",
            A = "";
        ...
        if (a = c.substring(0, 2), o = {
                ...
                GB: "[A-Z]{4}\\d{14}",
                VG: "[\\dA-Z]{4}\\d{16}"
            }, void 0 !== (r = o[a]) && (s = new RegExp("^[A-Z]{2}\\d{2}" + r + "$", ""), !s.test(c))) return !1;
        for (d = c.substring(4, c.length) + c.substring(0, 4), l = 0; l < d.length; l++) i = d.charAt(l), "0" !== i && (f = !1), f || (h += "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ".indexOf(i));
        for (u = 0; u < h.length; u++) n = h.charAt(u), A = "" + p + n, p = A % 97;
        return 1 === p
        ...
        return t = t.toString().toUpperCase(), !(t.length > 10 || t.length < 9 || !d.test(t)) && (t = t.replace(/^[X]/, "0").replace(/^[Y]/, "1").replace(/^[Z]/, "2"), a = 9 === t.length ? t.substr(0, 8) : t.substr(0, 9), i.charAt(parseInt(a, 10) % 23) === n)
